...
class SettingsController {
  ...
  Future<Response> getProductAttributes(Request request) async {
    final db = DatabaseService();
    final conn = await db.getConnection();
    try {
      final params = request.requestedUri.queryParameters;
      final String? companyIdRaw = params['company_id'];

      var results;
      if (companyIdRaw != null) {
        results = await conn.query(
          'SELECT DISTINCT attribute_name FROM product_attributes WHERE company_id = ?',
          [int.parse(companyIdRaw)],
        );
      } else {
        results = await conn.query(
          'SELECT DISTINCT attribute_name FROM product_attributes',
        );
      }

      final attributes = results.map((row) => row[0].toString()).toList();
      return Response.ok(
