...
class SalesController {
  ...
  Future<Response> recordSale(Request request) async {
    ...
    try {
      ...
      final int? providedId = data['sale_id'] ?? data['id'];
      var saleResult;
      if (providedId != null) {
        saleResult = await conn.query(
          'INSERT INTO sales (id, branch_id, user_id, staff_id, total_amount, customer_name, sale_date) VALUES (?, ?, ?, ?, ?, ?, NOW())',
          [providedId, branchId, userId, staffId, totalAmount, customerName],
        );
      } else {
        saleResult = await conn.query(
      ...
      final int? saleId = providedId ?? saleResult.insertId;
  ...
  Future<Response> getBranchSales(Request request) async {
    final db = DatabaseService();
    final conn = await db.getConnection();
    try {
      int? branchId;
      ...
      final sales = results.map((row) {
        final int saleId = row[9];
        final int productId = row[0];
        final int compositeId = (saleId * 10000) + productId;
