...
class ProductController {
    ...
    try {
      ...
      if (branchId == null) {
        if (request.method == 'POST') {
          ...
          if (bodyStr.isNotEmpty) {
            ...
            branchId = int.tryParse(data['branch_id']?.toString() ?? '');
          }
        } else {
      ...
      if (branchId == 0) {
        results = await conn.query(
          '''
          SELECT id, name, brand, category, specification, selling_price, current_stock, branch_id, cost_price 
          FROM products 
          WHERE is_deleted = 0
          '''
        );
  ...
  Future<Response> addProduct(Request request) async {
    ...
    try {
      ...
      final int? providedId = data['product_id'] ?? data['id'];
      var result;
      if (providedId != null) {
        result = await conn.query(
          '''
          INSERT INTO products 
          (id, branch_id, name, brand, category, specification, cost_price, selling_price, current_stock, total_inventory, low_stock_alert, high_stock_alert, is_deleted) 
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
          ON DUPLICATE KEY UPDATE
            ...
            is_deleted = 0
          ''',
          [providedId, branchId, name, brand, category, spec, cPrice, sPrice, stock, stock, lowAlert, highAlert],
        );
      } else {
        result = await conn.query(
          '''
      ...
      return Response.ok(
        jsonEncode({'status': 'success', 'id': providedId ?? result.insertId}), 
        headers: {'content-type': 'application/json'}
      );
    } catch (e) {
      return Response.internalServerError(body: jsonEncode({'error': e.toString()}));
  ...
  Future<Response> deleteProduct(Request request) async {
    ...
    try {
      final Map<String, dynamic> data = jsonDecode(await request.readAsString());
      final int? productId = int.tryParse(data['product_id']?.toString() ?? '');

      if (productId == null) {
        return Response.badRequest(
          body: jsonEncode({'error': 'Invalid or missing product_id'}),
          headers: {'content-type': 'application/json'},
        );
      }
      ...
      return Response.ok(
        jsonEncode({'status': 'success', 'message': 'Product deleted successfully'}),
        headers: {'content-type': 'application/json'}
      );
    } catch (e) {
      return Response.internalServerError(body: jsonEncode({'error': e.toString()}));
